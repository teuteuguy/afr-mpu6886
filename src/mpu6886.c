...
float prvARes, prvGRes;

/*-----------------------------------------------------------*/

void prvGetMPU6886Gres( void );
void prvGetMPU6886Ares( void );
...
mpu6886_err_t eMPU6886Init( IotI2CHandle_t handle )
{
    ...
    e = eReadI2CBytes( handle, MPU6886_I2C_ADDRESS, 1, &tempdata );
    ...
    for (i = 0; i < MPU6886_INIT_REG_COUNT * 2; i += 2)
    {
        ...
        tempdata[ 1 ] = mpu6886_init_register_default_list[ i + 1 ];
        e = eWriteI2CBytes( handle, MPU6886_I2C_ADDRESS, 2, &tempdata );
...
void prvGetMPU6886Gres( void )
{
    switch ( Gyscale )
    {
        ...
        case GFS_250DPS:
            prvGRes = 250.0 / 32768.0;
            break;
        case GFS_500DPS:
            prvGRes = 500.0 / 32768.0;
            break;
        case GFS_1000DPS:
            prvGRes = 1000.0 / 32768.0;
            break;
        case GFS_2000DPS:
            prvGRes = 2000.0 / 32768.0;
            break;
    }
}
...
mpu6886_err_t prvGetMPU6886AccelAdc( int16_t * ax, int16_t  *ay, int16_t * az )
{
    ...
    mpu6886_err_t e = eReadI2CBytes( prvI2CHandle, MPU6886_I2C_ADDRESS, buf, 6 );
...
mpu6886_err_t prvGetMPU6886TempAdc( int16_t * t )
{
    ...
    mpu6886_err_t e = eReadI2CBytes( prvI2CHandle, MPU6886_I2C_ADDRESS, buf, 2 );
...
mpu6886_err_t eGetMPU6886AhrsData( float * pitch, float * roll, float * yaw )
{
    ...
    e = eGetMPU6886yroData( &gyroX, &gyroY, &gyroZ );
...
mpu6886_err_t eSetMPU6886FIFOEnable( bool enableflag )
{
    ...
    if (enableflag)
    {
        ...
        regdata[ 1 ] = 0x0C;
        e = eWriteI2CBytes( prvI2CHandle, MPU6886_I2C_ADDRESS, &regdata, 2 );
        if ( e != COMMON_IO_SUCCESS )
        {
            return e;
        }
...
mpu6886_err_t eReadMPU6886FIFO( uint8_t * rd_data )
{
    ...
    return eReadI2CBytes( prvI2CHandle, MPU6886_I2C_ADDRESS, rd_data, 1 );
}
...
{
    rd_data[ 0 ] = MPU6886_REG_FIFO_R_W;
    return eReadI2CBytes( prvI2CHandle, MPU6886_I2C_ADDRESS, rd_data, length );
}
...
mpu6886_err_t eReadMPU6886FIFOCount( uint16_t * rd_data )
{
    ...
    mpu6886_err_t e = eReadI2CBytes( prvI2CHandle, MPU6886_I2C_ADDRESS, temp, 2 );
    *rd_data = temp[ 0 ];
    *rd_data <<= 8;
    *rd_data |= temp[ 1 ];
    return e;
...
mpu6886_err_t eSetMPU6886GyroFsr( enum Gscale scale )
{
    unsigned char regdata[ 2 ] = { MPU6886_REG_GYRO_CONFIG, ( scale << 3 ) };
    
    mpu6886_err_t e = eWriteI2CBytes( prvI2CHandle, MPU6886_I2C_ADDRESS, &regdata, 2 );
...
mpu6886_err_t eSetMPU6886AccelFsr( enum Ascale scale )
{
